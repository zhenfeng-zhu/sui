...
use move_binary_format::{
    ...
    file_format::{
        AbilitySet, CompiledModule, FunctionDefinitionIndex, FunctionHandleIndex, LocalIndex,
...
>(
    ...
    let module = vm
        ...
        .map_err(|e| convert_vm_error(e, with_vm_resolver(vm, state_view)))?;
...
>(
    ...
    for (idx, ty) in type_args.iter().enumerate() {
        session
            .load_type(ty)
            .map_err(|e| convert_type_argument_error(idx, e, with_vm_resolver(vm, state_view)))?;
    }
    ...
    let mut modules = module_bytes
        ...
        .map_err(|e| {
            ExecutionError::new_with_source(
                sui_types::messages::ExecutionFailureStatus::VMInvariantViolation,
                e,
            )
        })?;
...
fn convert_type_argument_error(
    idx: usize,
    error: VMError,
    vm_resolver: impl Fn(&ModuleId, FunctionDefinitionIndex) -> Option<Identifier>,
) -> ExecutionError {
...
>(
    vm: &'r MoveVM,
    state_view: &'r S,
) -> impl Fn(&ModuleId, FunctionDefinitionIndex) -> Option<Identifier> + 'r {
    |m: &ModuleId, idx: FunctionDefinitionIndex| {
        let module = vm.load_module(m, state_view).ok()?;
