...
pub fn convert_vm_error(
    error: VMError,
    resolve_fname: impl Fn(&ModuleId, FunctionDefinitionIndex) -> Option<Identifier>,
) -> ExecutionError {
        ...
        (StatusCode::ABORTED, Some(code), Location::Module(id)) => {
            ...
            ExecutionFailureStatus::MoveAbort(
                MoveLocation {
                    ...
                    function,
                    instruction,
                    function_name: resolve_fname(id, FunctionDefinitionIndex::new(function))
                ...
                let location = match location {
                    Location::Module(id) => {
                        ...
                        Some(MoveLocation {
                            ...
                            function,
                            instruction,
                            function_name: resolve_fname(
                                id,
                                FunctionDefinitionIndex::new(function),
                            )
                            .map(Identifier::into_string),
                        })
                    }
